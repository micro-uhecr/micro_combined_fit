...
def acceptance_func(p, arr):
    ...
    '''
    dx = 20
    x = np.arange(int(np.min(arr)-(dx/2)),int(np.max(arr)+(dx/2)),dx, dtype = float)
...
def resolution_func(p):
    ...
    r = np.arange(0,1,dx, dtype=float)
...
def convolve_diff_slow(xmax,model,res,x, acc):
    ...
    for j,e in enumerate(xmax):
        ...
        for jx,je in enumerate(xmax):
            meanxmax_2 = xmax[jx]
            window = np.linspace(meanxmax_2-meanxmax-(0.5*(dx)),meanxmax_2-meanxmax+(0.5*(dx)))
            resfun = np.trapz(reso_function(window), window)

            conv_func[jx][j] = model[j]*dx*resfun*acc[j]
...
def Convolve_all(xmax,A_tot, arr, model):
    ...
    for i in range( len(meanLgE)):
        ...
        for j in range(len(A_tot)):
            if A_tot[j] > 0:
                min = int((np.min(arr[i])-(dx/2))/dx)
                max = int((np.max(arr[i])+(dx/2))/dx)
