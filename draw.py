...
def Draw_spectrum_mod(A, logE, expected_spectrum, spectrum_per_mass, norm, E_fit, hadr_model, Dev = None, lEmin = 17.7, lEmax = 20.3, isInjected  = True, isE3dJdE= True, isSysDisplayed=False, saveTitlePlot=None):
    ''' Plot the expected and the experimental spectrum above the threshold energy
    ...
    sum = spectrum_per_mass[0]+ spectrum_per_mass[1]+spectrum_per_mass[2]+spectrum_per_mass[3]#+spectrum_per_mass[4]
    spectrum_per_mass[0] /= sum
    spectrum_per_mass[1] /= sum
    spectrum_per_mass[2] /= sum
    spectrum_per_mass[3] /= sum
    #spectrum_per_mass[4] /= sum

    filename = "../frac/proton_fractions.txt"
    p = Table.read(filename, format='ascii.basic', delimiter=",", guess=False)
    plt.errorbar(p['logE'], p['frac'], fmt='o',  mfc='none', color='red')

    filename = "../frac/helium_fractions.txt"
    He = Table.read(filename, format='ascii.basic', delimiter=",", guess=False)
    plt.errorbar(He['logE'], He['frac'], fmt='o',  mfc='none', color='grey')

    filename = "../frac/nitrogen_fractions.txt"
    N = Table.read(filename, format='ascii.basic', delimiter=",", guess=False)
    plt.errorbar(N['logE'], N['frac'], fmt='o',  mfc='none', color='green')

    filename = "../frac/iron_fractions.txt"
    Fe = Table.read(filename, format='ascii.basic', delimiter=",", guess=False)
    plt.ylabel('fraction')
    plt.xlabel(r'$\log_{10}(E/eV)$')
    plt.xlim(18.7,20)
    #plt.errorbar(Fe['logE'], Fe['frac'], fmt='o',  mfc='none', color='blue')



    for i, e in enumerate(spectrum_per_mass):
         if (i <3):
             plt.plot(logE, spectrum_per_mass[i], color=color_mass[i])

    plt.plot(logE, spectrum_per_mass[0]+spectrum_per_mass[1]+spectrum_per_mass[2]+spectrum_per_mass[3], color='black')

    fig2, axs = plt.subplots(figsize=(6, 4), nrows=4, ncols = 1, sharex=True)
    plt.subplots_adjust(bottom = 0.15, top = 0.92, left=0.15, right=0.96, hspace=0.0)

    axs[0].set_xlim(18.7,20)
    axs[0].set_ylim(-0.02,1)

    axs[0].errorbar(p['logE'], p['frac'], fmt='o',  mfc='none', color='red')
    axs[0].plot(logE, spectrum_per_mass[0], color=color_mass[0])
    axs[0].set_yticks([0, 0.5,1])

    #axs[0].axvline(x=18.7)


    axs[1].errorbar(He['logE'], He['frac'], fmt='o',  mfc='none', color='grey')
    axs[1].plot(logE, spectrum_per_mass[1], color=color_mass[1])
    #axs[1].axvline(x=18.7)
    axs[1].set_ylim(-0.02,1)
    axs[1].set_yticks([0, 0.5])

    axs[2].errorbar(N['logE'], N['frac'], fmt='o',  mfc='none', color='green')
    axs[2].plot(logE, spectrum_per_mass[2], color=color_mass[2])
    #axs[2].axvline(x=18.7)
    ...
    axs[3].errorbar(Fe['logE'], Fe['frac'], fmt='o',  mfc='none', color='blue')
    axs[3].plot(logE, spectrum_per_mass[3], color=color_mass[4])
    #axs[3].axvline(x=18.7)
    #axs[3].set_ylabel('fraction')
    axs[3].set_xlabel(r'$\log_{10}(E/eV)$')
    axs[3].set_ylim(-0.02,1)
    axs[3].set_yticks([0, 0.5])
