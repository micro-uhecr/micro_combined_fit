...
def Spectrum_Energy(Z, logR, gamma, logRcut, logEmin=0):
	...
	logEmin : `float`
		log minimum Energy for  the spectrum (below, it is 0)
	...
	#Normalization of the specrum
	ind_below_Emin = np.where(E<=Emin)
	weights_Normalization = np.copy(weights)
	weights_Normalization[ind_below_Emin] = 0
	weights = weights/np.sum(weights_Normalization*E, axis=0)
